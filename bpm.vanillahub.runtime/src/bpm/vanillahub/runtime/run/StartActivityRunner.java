...
public class StartActivityRunner extends ActivityRunner<StartActivity> {

	public StartActivityRunner(WorkflowProgress workflowProgress, IVanillaLogger logger, String workflowName, String launcher, StartActivity activity, List<Parameter> parameters, List<Variable> variables, boolean isLoop) {
		super(workflowProgress, logger, workflowName, launcher, activity, parameters, variables, isLoop);
	}

	@Override
	public void run(Locale locale) {
		addInfo(Labels.getLabel(locale, Labels.StartWorkflow));
