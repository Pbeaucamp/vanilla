...
public class AdminServlet extends HttpServlet {
	...
	public AdminServlet(ComponentVanillaHub component) {
	...
	private String connect(User user, HttpServletRequest req, HttpServletResponse resp, Locale loc) throws Exception {
		HubSession session = new HubSession(component, loc);
		session = component.getSessionHolder().createSession(session, user);
		req.setAttribute(RemoteConstants.HTTP_HEADER_SESSION_ID, session.getSessionId());
		req.getSession().setAttribute(RemoteConstants.HTTP_HEADER_SESSION_ID, session.getSessionId());
		
		resp.addHeader(RemoteConstants.HTTP_HEADER_SESSION_ID, session.getSessionId());
