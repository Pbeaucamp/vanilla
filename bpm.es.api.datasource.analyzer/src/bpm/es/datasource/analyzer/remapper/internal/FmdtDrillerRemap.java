...
public class FmdtDrillerRemap implements IRemapperPerformer{
	...
	public FmdtDrillerRemap(Document sourceXmlDocument, RepositoryItem orginialItem, RepositoryItem newItem){
...
//		XPath xpath = new Dom4jXPath( "//oda-data-source[@extensionID='bpm.metadata.birt.oda.runtime']|//oda-data-source[@extensionID='bpm.csv.oda.runtime']|//oda-data-source[@extensionID='bpm.excel.oda.runtime']" );
		...
		for(Element e : (List<Element>)root.selectNodes("/metadata")){
	...
	public String getTaskPerfomed() {
		return "Replace FMDT Items References  " + orginialItem.getItemName() + " by " + newItem.getItemName() + " in FMDT Driller model";
	}
}
