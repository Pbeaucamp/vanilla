...
public class TaskListRemap implements IRemapperPerformer{
	...
	public TaskListRemap(Document sourceXmlDocument, RepositoryItem orginialItem, RepositoryItem newItem){
...
//		XPath xpath = new Dom4jXPath( "//oda-data-source[@extensionID='bpm.metadata.birt.oda.runtime']|//oda-data-source[@extensionID='bpm.csv.oda.runtime']|//oda-data-source[@extensionID='bpm.excel.oda.runtime']" );
		...
		for(Element e : (List<Element>)root.selectNodes("//task/property[@name='directpryItemId']")){
	...
	public String getTaskPerfomed() {
		return "Replace Items References " + orginialItem.getItemName() + " by " + newItem.getItemName() + " in TaskList  model";
	}
}
