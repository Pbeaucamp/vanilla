package bpm.vanilla.workplace.client.tab;
...
public class CustomTabLayoutPanel extends ResizeComposite implements HasWidgets,
  ...
  private class Tab extends SimplePanel {
    ...
    public void setSelected(boolean selected) {
      if (selected) {
    	addStyleName("tabSelected");
        addStyleDependentName("selected");
      } 
      else {
    	removeStyleName("tabSelected");
  ...
  private static final String CONTENT_CONTAINER_STYLE_BOTTOM = "gwt-TabLayoutPanelContentContainerBottom";
  private static final String CONTENT_STYLE_BOTTOM = "gwt-TabLayoutPanelContentBottom";
  private static final String TAB_STYLE_BOTTOM = "gwt-TabLayoutPanelTabBottom";
   ...
   */
  public CustomTabLayoutPanel(double barHeight, Unit barUnit, boolean tabOnSide) {
	this.tabOnSide = tabOnSide;
    ...
    panel.add(tabBar);
    panel.setWidgetLeftRight(tabBar, 25, Unit.PX, 25, Unit.PX);
    if(tabOnSide){
    	panel.setWidgetLeftWidth(tabBar, 0, Unit.PX, barHeight, barUnit);
        // Add the deck panel to the panel.
        deckPanel.addStyleName(CONTENT_CONTAINER_STYLE_BOTTOM);
    }
    else {
    ...
    if(tabOnSide){
    	panel.setWidgetLeftRight(deckPanel, barHeight, Unit.PX, 0, barUnit);
    	deckPanel.getElement().getStyle().setBackgroundColor("#FFFFFF");
    }
    else {
    ...
    tabBar.getElement().getStyle().setHeight(BIG_ENOUGH_TO_NOT_WRAP, Unit.PX);
  ...
  private void insert(final Widget child, Tab tab, int beforeIndex) {
    ...
    if(tabOnSide){
    	child.addStyleName(CONTENT_STYLE_BOTTOM);
    }
    else {
