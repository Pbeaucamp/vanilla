...
public class HBaseOutputDefinitionSection extends AbstractPropertySection {

	private HBaseOutputStream model;
	
	private Label lblFamily;
	private Text targetName, targetColumnFamily;
	private Button truncateRun;
	
	public HBaseOutputDefinitionSection() { }
	...
	public void setInput(IWorkbenchPart part, ISelection selection) {
        ...
        this.model = (HBaseOutputStream)((Node)((NodePart) input).getModel()).getGatewayModel();
	}
	...
	public void createControls(Composite parent,
		...
		lblFamily = getWidgetFactory().createLabel(composite, Messages.HBaseOutputDefinitionSection_0);
		lblFamily.setLayoutData(new GridData(GridData.BEGINNING, GridData.CENTER, false, false));
		
		targetColumnFamily = getWidgetFactory().createText(composite, ""); //$NON-NLS-1$
		targetColumnFamily.setLayoutData(new GridData(GridData.FILL, GridData.CENTER, true, false));
		...
		Button bApply = getWidgetFactory().createButton(composite, Messages.HBaseOutputDefinitionSection_1, SWT.PUSH);
		...
		bApply.addSelectionListener(new SelectionAdapter(){
			...
			public void widgetSelected(SelectionEvent e) {
				...
				model.setColumnFamily(targetColumnFamily.getText());
		...
		bTruncate.addSelectionListener(new SelectionAdapter(){
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if(MessageDialog.openConfirm(sh, Messages.HBaseOutputDefinitionSection_2, Messages.HBaseOutputDefinitionSection_3)){
					if (model.getServer() == null){
	...
	public void refresh() {
		...
		if (model.getColumnFamily() != null){
			targetColumnFamily.setText(model.getColumnFamily());
		}
		else{
			targetColumnFamily.setText(""); //$NON-NLS-1$
		}
