...
//	private Button c0n, c01,c1n, c11;
	...
	private void buildContent(){
		...
		leftOuter.setText(Messages.CompositeRelation_6);
		...
		rightOuter.setText(Messages.CompositeRelation_7);
...
//		});
		...
		add.setText(Messages.CompositeRelation_2); //$NON-NLS-1$
		add.addSelectionListener(new SelectionAdapter(){
			...
			public void widgetSelected(SelectionEvent e) {
				...
				try {
					...
					MessageDialog.openWarning(getShell(), Messages.CompositeRelation_3, e1.getMessage()); //$NON-NLS-1$
				} catch (RelationException e1) {
					Activator.getLogger().error(e1.getMessage(), e1);
					MessageDialog.openWarning(getShell(), Messages.CompositeRelation_4, e1.getMessage()); //$NON-NLS-1$
				}
		...
		Button browseRel = new Button(bar, SWT.PUSH);
		browseRel.setLayoutData(new GridData(GridData.CENTER, GridData.CENTER, false, false));
		browseRel.setText("..."); //$NON-NLS-1$
		browseRel.addSelectionListener(new SelectionAdapter(){
			@Override
			public void widgetSelected(SelectionEvent e) {
				...
				DialogBusinessTable d = new DialogBusinessTable(getShell(), table);
				d.open();
		...
		del.setText(Messages.CompositeRelation_5); //$NON-NLS-1$
		...
		labelRelation.setText("Final relation");
	...
	private void refreshTextQuery(boolean init) {
		...
		if(relation.isOuterJoin()) {
			...
			if(init && relation.getFinalRelation() != null && !relation.getFinalRelation().isEmpty() && !relation.getFinalRelation().equals("null")) {
