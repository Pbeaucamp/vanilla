package bpm.es.client.splashHandlers;
...
public class InteractiveSplashHandler extends AbstractSplashHandler {
	...
	private void createUICompositeLogin() {
		...
		fButtonOK.setText(Messages.InteractiveSplashHandler_0);
		fButtonOK.setLayoutData(dataOk);
		fButtonOK.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				handleButtonOKWidgetSelected();
//				Program.launch("http://www.google.fr");
			}
		});
		...
		fButtonCancel.setText(Messages.InteractiveSplashHandler_1);
	...
	private void fill() {
		...
		String login = s != null && s.getString(PreferenceConstants.P_BI_REPOSITIRY_USER) != null && !s.getString(PreferenceConstants.P_BI_REPOSITIRY_USER).isEmpty() ? s.getString(PreferenceConstants.P_BI_REPOSITIRY_USER) : "system";
		String password = s != null && s.getString(PreferenceConstants.P_BI_REPOSITIRY_PASSWORD) != null && !s.getString(PreferenceConstants.P_BI_REPOSITIRY_PASSWORD).isEmpty() ? s.getString(PreferenceConstants.P_BI_REPOSITIRY_PASSWORD) : "system";
		String vanillaUrl = s != null && s.getString(PreferenceConstants.p_BI_SECURITY_SERVER) != null && !s.getString(PreferenceConstants.p_BI_SECURITY_SERVER).isEmpty() ? s.getString(PreferenceConstants.p_BI_SECURITY_SERVER) : "http://localhost:7171/VanillaRuntime";

		txtLogin.setText(login);
		txtPassword.setText(password);
		txtVanillaUrl.setText(vanillaUrl);
	}
	...
	private void initRepositories() {
		try {
			...
			User user = tmp.getVanillaSecurityManager().authentify("", login, password, false); //$NON-NLS-1$
	...
	protected void initManager() {
		try {
			...
			if(vanillaApi.getVanillaSecurityManager().canAccessApp(-1, Activator.SOFT_ID)) {
				...
				MessageDialog.openError(getSplash().getShell(), Messages.DialogRepository_15, Messages.DialogRepository_16);
			}
		} catch (Exception e) {
			e.printStackTrace();
