...
public class FusionmapsPageGenerator extends AbstractPageGenerator{

	private static final String CUSTOM_PAGE_TITLE = "Custom"; //$NON-NLS-1$
	private IPropertyTabUI generalPage;
	private IPropertyTabUI bindingPage;
	...
	protected void buildItemContent( CTabItem item ) {
		if ( itemMap.containsKey( item ) && itemMap.get( item ) == null )
		{
			...
			{
				TabPage page = new FusionmapsGeneralPage( ).getPage( );
				if ( page != null )
				{
					setPageInput( page );
	...
	public void refresh( )
	{
		...
		bindingPage.setInput( input );
		addSelectionListener( this );
		( (TabPage) bindingPage ).refresh( );
	}
	
	public void createTabItems( List input )
	{
		if ( bindingPage == null || bindingPage.getControl( ).isDisposed( ) )
		{
			...
			generalPage = AttributesUtil.buildGeneralPage( tabFolder,
				...
				new String[]{ "General" }, //$NON-NLS-1$
				new AttributesUtil.PageWrapper[]{ new FusionmapsGeneralPage( )	},
				input );
			bindingPage = AttributesUtil.buildBindingPage(tabFolder, input);
			
			CTabItem tabItem = new CTabItem( tabFolder, SWT.NONE );
			tabItem.setText( ATTRIBUTESTITLE );
			tabItem.setControl( bindingPage.getControl( ) );
		}
