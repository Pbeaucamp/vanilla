...
public class PreviewReportServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String checkout = req.getParameter(CommonConstants.CHECKOUT);
		String bigFile = req.getParameter(CommonConstants.REPORT_BIG_FILE);
...
//			}
			...
			if ((checkout != null && checkout.equalsIgnoreCase("true")) || (bigFile != null && bigFile.equalsIgnoreCase("true"))) {
				resp.setHeader("Content-disposition", "attachment; filename=" + name + "." + format);
			}
			else if(format.equalsIgnoreCase("pht")) {
				resp.setHeader("Content-disposition", "attachment; filename=" + name + "." + format);
			}
			else {
