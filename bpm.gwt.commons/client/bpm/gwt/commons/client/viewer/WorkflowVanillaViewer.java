...
public class WorkflowVanillaViewer extends Viewer {

	private VanillaViewer vanillaViewer;
	private LaunchReportInformations itemInfo;
	
	public WorkflowVanillaViewer(VanillaViewer vanillaViewer, PortailRepositoryItem item, Group selectedGroup, boolean forceOpen) {
		super(vanillaViewer);
	...
	public void runItem(LaunchReportInformations itemInfo) {
		...
		ReportingService.Connect.getInstance().runWorkflow(itemInfo, new AsyncCallback<String>() {
			@Override
			...
			@Override
			public void onSuccess(String wfUrl) {
				showWaitPart(false);
	...
	private void showWorkflow(String gtwUrl) {
		reportFrame.setUrl(gtwUrl);
	}

	@Override
	public void onRunClick(ClickEvent event) {
		vanillaViewer.launchReport(this, itemInfo.getItem(), itemInfo.getSelectedGroup(), false, false);
	}
