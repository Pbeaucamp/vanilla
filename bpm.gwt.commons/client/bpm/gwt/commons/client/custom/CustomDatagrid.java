...
public class CustomDatagrid<T> extends SimplePanel {
	...
	public CustomDatagrid(List<T> groups, MultiSelectionModel<T> multiSelectionModel, int height, String emptyMessage) {
		this.selectionModel = multiSelectionModel;
		setWidget(buildGrid(groups, height + "px", true, emptyMessage, LabelsConstants.lblCnst.Groups()));
	}
	...
	private DataGrid<T> buildGrid(List<T> groups, String height, boolean isShowOption, String emptyMessage, final String header) {
		...
		dataGrid = new DataGrid<T>(10000, resources);
	...
	public void addColumn(Column<T, String> column, String header, String width) {
		if (header != null) {
	...
	public void addCheckColumn(Column<T, CheckState> column, String header, String width) {
		if (header != null) {
			dataGrid.addColumn(column, header);
		}
		else {
