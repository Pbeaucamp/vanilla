...
public class DisableableCheckboxCell extends AbstractEditableCell<Boolean, Boolean> {
	 ...
	 */
	private DisableableCheckboxCell(boolean dependsOnSelection, boolean handlesSelection) {
	...
	public void onBrowserEvent(Context context, Element parent, Boolean value, NativeEvent event, ValueUpdater<Boolean> valueUpdater) {
		...
		if (BrowserEvents.CHANGE.equals(type) || enterPressed) {
			...
			/*
			 * Toggle the value if the enter key was pressed and the cell handles selection or doesn't depend on selection. If the cell depends on selection but doesn't handle selection, then ignore the enter key and let the SelectionEventManager determine which keys will trigger a change.
			 */
