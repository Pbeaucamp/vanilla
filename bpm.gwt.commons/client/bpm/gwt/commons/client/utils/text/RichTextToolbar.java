...
import bpm.gwt.commons.client.utils.text.OnlyHtmlDialog.IHtmlChangeHandler;
...
public class RichTextToolbar extends Composite {
	...
	/**
	 * This {@link ClientBundle} is used for all the button icons. Using a bundle allows all of these images to be packed into a single image, which saves a lot of HTTP requests, drastically improving startup time.
	 */
	...
	private class EventHandler implements ClickHandler, ChangeHandler, KeyUpHandler {
		...
		public void onClick(ClickEvent event) {
			...
			else if (sender == openHtmlOnly) {
				String html = richText.getHTML();

				OnlyHtmlDialog dial = new OnlyHtmlDialog(htmlChangeHandler, html);
				dial.center();
			}
	...
	private PushButton openHtmlOnly;
	 ...
	 *            the rich text area to be controlled
	 */
	public RichTextToolbar(RichTextArea richText, IHtmlChangeHandler htmlChangeHandler) {
		this.htmlChangeHandler = htmlChangeHandler;
		buildContent(richText);
	}

	private void buildContent(RichTextArea richText) {
		...
		if (htmlChangeHandler != null) {
			topPanel.add(openHtmlOnly = createPushButton(CommonImages.INSTANCE.HTML_20(), LabelsConstants.lblCnst.HtmlEditor()));
		}
