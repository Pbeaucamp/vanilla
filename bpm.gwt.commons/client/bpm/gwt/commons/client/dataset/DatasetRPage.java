...
public class DatasetRPage extends DatasetPanel implements IGwtPage {

	private static DatasetRPageUiBinder uiBinder = GWT.create(DatasetRPageUiBinder.class);

	interface DatasetRPageUiBinder extends UiBinder<Widget, DatasetRPage> {
	}
	...
	public DatasetRPage(DatasetWizard parent, Datasource datasource, Dataset dataset) {
	...
	private void loadRMetaData(){

		CommonService.Connect.getInstance().getRDataSetMetaData(dataset, parent.getUser().getLogin()+parent.getUser().getId(), new AsyncCallback<ArrayList<DataColumn>>() {	
			@Override
	...
	public String getQuery(String datasetName) {
		...
		DatasourceR rdts = (DatasourceR) dataset.getDatasource().getObject();
		String query = "library(" + rdts.getPackageR() + ")\n";
		query += "data("+ rdts.getOriginalDatasetR() +")\n";
		query += datasetName + " <- " + rdts.getOriginalDatasetR() + "[c(" + cols + ")]";
		return query;
	}
}
