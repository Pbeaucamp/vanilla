...
public class LaunchReportInformations {
	...
	//Not used by Ged_entry item
	private List<GedItem> availableGeds;
	...
	public LaunchReportInformations(PortailRepositoryItem item, Group selectedGroup, List<VanillaGroupParameter> groupParameters, 
			...
			List<String> reportResources, AlternateDataSourceHolder alternateDatasource, List<Group> availableGroups, List<ItemMetadataTableLink> metadataLinks,List<GedDocument> availableDocuments) {
		this.item = item;
		...
		this.availableGeds = new ArrayList<>();
		for(GedDocument gedDoc : availableDocuments) {
			this.availableGeds.add(new GedItem(gedDoc));
		}
	}
	...
	public List<GedItem> getAvailableGeds() {
		return availableGeds;
	}

	public void setAvailableGeds(List<GedItem> availableDocuments) {
		this.availableGeds = availableDocuments;
	}
