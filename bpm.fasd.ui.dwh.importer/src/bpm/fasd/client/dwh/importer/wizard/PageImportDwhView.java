...
public class PageImportDwhView extends WizardPage{
	...
	private void createContent(Composite parent){
		...
		try{
			...
			itemSelecter.setInput(new Repository(FreemetricsPlugin.getDefault().getRepositoryConnection(),IRepositoryApi.DWH_VIEW_TYPE));
		}catch(Exception ex){
			ex.printStackTrace();
			MessageDialog.openError(getShell(), Messages.PageImportDwhView_0, Messages.PageImportDwhView_1 + ex.getMessage());
		}
	}
