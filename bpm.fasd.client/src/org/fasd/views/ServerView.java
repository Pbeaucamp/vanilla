...
public class ServerView extends ViewPart {
	public static final String ID = "org.fasd.views.serverview"; //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
		...
		viewer.setLabelProvider(new LabelProvider(){
			...
			public String getText(Object element) {
				if (element instanceof ServerConnection){
					return ((ServerConnection)element).getName();
				}
				return null;
			}
		...
		viewer.setContentProvider(new IStructuredContentProvider(){
			...
			public Object[] getElements(Object inputElement) {
				ServerConnection[] data = (ServerConnection[])inputElement;
				return data;
			}
...
//		listenner
	...
	public void setFocus() {
		if (FreemetricsPlugin.getDefault().isRepositoryConnectionDefined()){
			getViewSite().getActionBars().getStatusLineManager().setMessage(LanguageText.ServerView_0 + FreemetricsPlugin.getDefault().getRepositoryConnection().getContext().getRepository().getUrl());
		}
		else{
			getViewSite().getActionBars().getStatusLineManager().setMessage(LanguageText.ServerView_1);
		}
		...
		tree.addSelectionChangedListener(new ISelectionChangedListener() {
			   public void selectionChanged(SelectionChangedEvent event) {
			       ...
			       if(event.getSelection() instanceof IStructuredSelection) {
			           ...
			           DetailView view = ((DetailView) ServerView.this.getSite().getWorkbenchWindow().getActivePage().findView(DetailView.ID));
			           view.selectionChanged(ServerView.this, selection); 
			}}});
	...
	public void refresh() {
		...
		List<ServerConnection> root = FreemetricsPlugin.getDefault().getSecurity().getServerConnections();
		viewer.setInput(root.toArray(new ServerConnection[root.size()]));
		
	}

	private void createActions(){
		Image img = new Image(getSite().getShell().getDisplay(),Platform.getInstallLocation().getURL().getPath() +"icons/add.png"); //$NON-NLS-1$
		add = new Action(LanguageText.ServerView_Add_Server_Connection, ImageDescriptor.createFromImage(img)){
			public void run(){
				Security s = FreemetricsPlugin.getDefault().getSecurity();
				s.addServerConnection(new ServerConnection(LanguageText.ServerView_New_Server_Connection));
		...
		del = new Action(LanguageText.ServerView_Del_Server_Connection, ImageDescriptor.createFromImage(img)){
			public void run(){
				Object o = ((StructuredSelection)viewer.getSelection()).getFirstElement();
				if (o instanceof ServerConnection){
					FreemetricsPlugin.getDefault().getSecurity().removeServerConnection((ServerConnection)o);
					FreemetricsPlugin.getDefault().getFAModel().setChanged();
				}
			}
		};
