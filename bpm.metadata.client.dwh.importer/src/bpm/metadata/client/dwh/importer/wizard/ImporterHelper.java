...
public class ImporterHelper {
	...
	public static DocumentSnapshot loadDwhView(RepositoryItem item) throws Exception{
		 ...
		 * load xml
		 */
		IRepositoryApi client = Activator.getDefault().getRepositoryConnection();
		String xml = null;
		try {
			xml = client.getRepositoryService().loadModel(item);
		} catch (Exception e) {
		...
		File tmp = new File(Platform.getInstallLocation().getURL().getPath()+ "/temp");
		if (!tmp.exists()){
			tmp.mkdirs();
		}
