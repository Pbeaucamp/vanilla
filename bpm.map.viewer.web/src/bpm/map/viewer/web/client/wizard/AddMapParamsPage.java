...
public class AddMapParamsPage extends Composite implements IGwtPage {
	...
	private void loadLevels() {
		...
		for(int i = 0; i<flexLevel.getRowCount(); i++){
		  for(int j =0; j<flexLevel.getCellCount(i); j++){
			  if(((Image)flexLevel.getWidget(i, j)).getUrl().contains(
					  (((selectedLevel.getIconUrl().contains("/"))? selectedLevel.getIconUrl().substring(selectedLevel.getIconUrl().lastIndexOf("/")):selectedLevel.getIconUrl()))
				)){
	...
	private void loadMetrics() {
		...
		for(int i = 0; i<flexMetric.getRowCount(); i++){
		  for(int j =0; j<flexMetric.getCellCount(i); j++){
			  if(((Image)flexMetric.getWidget(i, j)).getUrl().contains(
					  (((selectedMetric.getIconUrl().contains("/"))? selectedMetric.getIconUrl().substring(selectedMetric.getIconUrl().lastIndexOf("/")):selectedMetric.getIconUrl()))
				)){
