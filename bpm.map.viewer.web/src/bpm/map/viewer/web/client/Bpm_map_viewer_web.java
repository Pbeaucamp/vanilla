...
public class Bpm_map_viewer_web extends RootWaitPanel implements EntryPoint, ILogin {
	...
	private static final String ID_MAP = "bpm.kpi.idMap";
	private static final String LEVELS = "bpm.kpi.levels";
	private static final String METRICS = "bpm.kpi.metrics";
	private static final String STARTDATE = "bpm.kpi.startdate";
	...
	private void initSession() {
		ConnectionServices.Connection.getInstance().initSession(new AsyncCallback<Void>() {
			...
			public void onSuccess(Void result) {
				...
				LoginHelper loginHelper = new LoginHelper(RootPanel.get("Login"), Bpm_map_viewer_web.this, Bpm_map_viewer_web.this, "VanillaMap_Logo_Accueil.png", false, true);
				loginHelper.loginToApplication(actualUrl);
				
				props = parseUrl(actualUrl);
	...
	private void buildContent(Keycloak keycloak) {
		TypeDisplay display;
		if(props.get(LEVELS) != null) {
			display = TypeDisplay.VIEWER;
			
		} else {
			display = TypeDisplay.NORMAL;
		}
		
		MainPanel mainPanel = new MainPanel(display, keycloak);
