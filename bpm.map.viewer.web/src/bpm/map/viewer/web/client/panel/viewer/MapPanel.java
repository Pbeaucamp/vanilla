...
public class MapPanel extends Composite {
	...
	public MapPanel(MapViewer parent, List<ComplexObjectViewer> result, List<ComplexMapMetric> displayedMetrics, final MapInformation mapInformation) {
...
//	}
//	
	public void loadMap(MapInformation mapInformation) {	
		...
		renderMap("mapContainer", obj, parent.getViewer().getActionDate().toString(), wmtsSource, wmsSource);	
	}
	...
	private final native void renderMap(String div, JSONObject obj, String action, String wmtsSource, JsArrayMixed wmsSource) /*-{		
		$wnd.renderComplexMapWithWms(div, obj, action, wmtsSource, wmsSource);
	}-*/;
	...
	public final native String getMapPrint() /*-{
		
		var x;
		$wnd.globalMap.once('postcompose', function(event) {
      		var canvas = event.context.canvas;
      		x = canvas.toDataURL('image/png');
    	});
    	$wnd.globalMap.renderSync();
