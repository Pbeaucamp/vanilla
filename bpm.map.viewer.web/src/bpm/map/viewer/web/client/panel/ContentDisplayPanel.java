...
public class ContentDisplayPanel extends Composite implements TabManager {
	...
	public ContentDisplayPanel(MainPanel mainPanel, TypeDisplay display) {
		initWidget(uiBinder.createAndBindUi(this));
		this.mainPanel = mainPanel;

		buildContentPanel(display);

	}

	private void buildContentPanel(TypeDisplay display) {
		if(display == TypeDisplay.NORMAL){
			if (viewerPanel == null) {
				viewerPanel = new ViewerPanel(this, display);
			}
			...
			contentPanel.addStyleName(style.contentPanelFull());
			if (viewerPanel == null) {
				viewerPanel = new ViewerPanel(this, display);
			}
			changeTab(viewerPanel);
	...
	private void changeTab(Tab selectedPanel) {
		...
		if(designerPanel != null) designerPanel.getDatagrid().redraw();
