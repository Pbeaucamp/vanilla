...
public class CrossRefDataLoader {
	...
	static {
		errorMess.put(1, "Not set");
		errorMess.put(2, "Failed to Load DirectoryItem list");
		errorMess.put(3, "Failed to Load DirectoryItem depends list");
		errorMess.put(4, "Failed to Load DirectoryItem needs list");
		errorMess.put(9, "Thread faild to start");
	}
	...
	class Loader extends Thread{
		...
		public void run() {
			try {
				// Load the full List
				setErrorCode(2);
				Repository rep = new Repository(Activator.getDefault().getRepositoryApi());
				setFullItemList(rep.getAllItems());
				//setFullItemList(((Repository)Activator.getDefault().repository).getAllItems());
				//System.out.println("list size : " + fullList.size());
				// Load the depends list
				setErrorCode(3);
	...
	public CrossRefDataLoader(){
		...
		//setFullItemList(new ArrayList<IDirectoryItem>());
		//setDepandsItemList(new HashMap<DirectoryItem, Collection<DirectoryItem>>());
		//setNeedsItemList(new HashMap<DirectoryItem, Collection<DirectoryItem>>());
		// Launch Data Loader
	...
	public String getErrorMessage(){
		return (getErrorCode() > 0)? errorMess.get(getErrorCode()):"";
	}
	/*
	 * LoadDate.
	 */
