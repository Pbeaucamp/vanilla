...
public class ExportLocation extends WizardPage implements Listener{
	...
	public ExportLocation(ExportModel model){
		super("Page name test");
		setTitle("Exporter vers FLEX");
		setDescription("Exporter le shema vers une application FLEX.");
		setModel(model);
	}
	...
	public void createControl(Composite parent) {
		...
		lab.setText("Selectionnez la racine du bin flex");
		...
		launch.setText("Executer l'application Flex");
		...
		if (event.widget == urlField){
			if (!checkURL()){
				status = new Status(IStatus.ERROR, "not_used", 0, 
				           "Wrong location : Cannot find valid Flex Application", null);	
			}
