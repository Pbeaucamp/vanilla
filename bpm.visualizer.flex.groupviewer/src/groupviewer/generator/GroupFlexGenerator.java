...
public class GroupFlexGenerator extends FlexGenerator{
	...
	private void generateGroupElements() {
		...
		for(Group group : groupList){
			Node node = new Node(group.getName(),group.getComment(),"group", "35");
			node.setData(group);
	...
	private void generateUserElements() {
		...
		for(User user : userList){
			Node node = new Node(user.getName(),user.getFunction(), "user", "25");
			node.setData(user);
	...
	public Map<Class<?>, List<FlexProperty>> createExtraAtrributes() {
		...
		usersProp.add(new FlexProperty("bMail", "getBusinessMail"));
		usersProp.add(new FlexProperty("cellular", "getCellular"));
		usersProp.add(new FlexProperty("fonction", "getFunction"));
		usersProp.add(new FlexProperty("login", "getLogin"));
		usersProp.add(new FlexProperty("pMail", "getPrivateMail"));
		usersProp.add(new FlexProperty("sName", "getSkypeName"));
		usersProp.add(new FlexProperty("sNumber", "sNumber"));
		usersProp.add(new FlexProperty("surname", "getSurname"));
		usersProp.add(new FlexProperty("phone", "getTelephone"));
