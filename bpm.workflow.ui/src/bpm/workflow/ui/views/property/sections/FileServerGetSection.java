...
/**
 * Section for the get... activities (file management)
 * 
 * @author Charles MARTIN
 * 
 */
public class FileServerGetSection extends AbstractPropertySection {
	private Combo server;

	private Node node;

	public FileServerGetSection() {}
	...
	public void createControls(Composite parent, TabbedPropertySheetPage tabbedPropertySheetPage) {
		...
		CLabel labelLabel = getWidgetFactory().createCLabel(parent, Messages.FileServerGetSection_0);
		labelLabel.setLayoutData(new GridData(GridData.BEGINNING, GridData.CENTER, false, false));

		server = new Combo(parent, SWT.READ_ONLY);
		...
		public void widgetSelected(SelectionEvent e) {
			...
			if(server.getText() != null) {
				...
				if(s != null && s instanceof FileServer) {
					((IFileServerGet) node.getWorkflowObject()).setServer(s);
