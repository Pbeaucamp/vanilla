...
public class DatasourceSection extends AbstractPropertySection {
	...
	public void createControls(Composite parent, TabbedPropertySheetPage tabbedPropertySheetPage) {
		...
		CLabel lblType = getWidgetFactory().createCLabel(composite, Messages.DatasourceSection_0);
		lblType.setLayoutData(new GridData(GridData.BEGINNING, GridData.CENTER, false, false));

		ds = new Combo(composite, SWT.READ_ONLY);
		ds.setLayoutData(new GridData(GridData.FILL, GridData.CENTER, false, false));
	...
	public void aboutToBeShown() {
		if(!ds.isListening(SWT.Selection))
			ds.addSelectionListener(adapter);
	...
	public void aboutToBeHidden() {
		ds.removeSelectionListener(adapter);
	...
	private SelectionAdapter adapter = new SelectionAdapter() {
		...
		public void widgetSelected(SelectionEvent e) {
			int dsid = new Integer(ds.getText()).intValue();
			((JrxmlReport) ((ReportActivity) node.getWorkflowObject()).getBiObject()).setDataSourceId(dsid);
		}
	};
