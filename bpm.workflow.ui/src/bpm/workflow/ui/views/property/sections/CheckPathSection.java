...
/**
 * Section for the Checkpath activity
 * @author Charles MARTIN
 *
 */
public class CheckPathSection extends AbstractPropertySection {
	...
	public void createControls(Composite parent,
		...
		general.setText(Messages.CheckPathSection_0);
		
		
		CLabel labelLabel = getWidgetFactory().createCLabel(general, Messages.CheckPathSection_1);
		...
		CLabel labelcom = getWidgetFactory().createCLabel(general, Messages.CheckPathSection_3);
		...
		select.setLayoutData(new GridData(GridData.FILL,GridData.CENTER,true, false, 2, 1));
		select.addModifyListener(new ModifyListener() {
			
			@Override
			public void modifyText(ModifyEvent e) {
				((CheckPathActivity)node.getWorkflowObject()).setPath(select.getText());
		...
		browse.setText(Messages.CheckPathSection_6);
		...
		browse.addSelectionListener(new SelectionAdapter(){
						...
						public void widgetSelected(SelectionEvent e) {
							FileServer server = (FileServer)((CheckPathActivity)node.getWorkflowObject()).getServer();
							...
							if(typeofServer.equalsIgnoreCase("Hard Drive")){ //$NON-NLS-1$
								...
								if(url.contains("{$VANILLA_HOME}")){ //$NON-NLS-1$
									MessageDialog.openInformation(Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell(), Messages.CheckPathSection_9, Messages.CheckPathSection_10);
									
								}
								if(url.contains("{$VANILLA_FILES}")){ //$NON-NLS-1$
									
									MessageDialog.openInformation(Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell(), Messages.CheckPathSection_12, Messages.CheckPathSection_13);
								...
								nom.add(Messages.CheckPathSection_15);
								...
								FTPClient ftp = new FTPClient();
							      ...
							      List<List<String>> resultat = new ArrayList<List<String>>();
									...
									ArrayList<String> nom = new ArrayList<String>();
									nom.add(Messages.CheckPathSection_18);
	...
	public void refresh() {
		...
		if(((CheckPathActivity)node.getWorkflowObject()).getPath() != null){
		
		select.setText(((CheckPathActivity)node.getWorkflowObject()).getPath());
		}
		...
		public void widgetSelected(SelectionEvent e) {
			...
			if (server.getText() != null) {
				...
				if (s!=null && s instanceof FileServer) {
					((CheckPathActivity) node.getWorkflowObject()).setServer(s);
