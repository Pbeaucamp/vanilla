...
/**
 * Section for the delete folder activity
 * 
 * @author Charles MARTIN
 * 
 */
public class DeleteFolderSection extends AbstractPropertySection {
	...
	public void createControls(Composite parent, TabbedPropertySheetPage tabbedPropertySheetPage) {
		...
		general.setText(Messages.DeleteFolderSection_0);

		CLabel labelLabel = getWidgetFactory().createCLabel(general, Messages.DeleteFolderSection_1);
		...
		CLabel labelcom = getWidgetFactory().createCLabel(general, Messages.DeleteFolderSection_3);
		...
		validPath.setText(Messages.DeleteFolderSection_2);

		validPath.addSelectionListener(new SelectionAdapter() {

			@Override
			public void widgetSelected(SelectionEvent e) {
				((DeleteFolderActivity) node.getWorkflowObject()).setPath(select.getText());
		...
		browse.setText(Messages.DeleteFolderSection_6);

		browse.addSelectionListener(new SelectionAdapter() {

			@Override
			public void widgetSelected(SelectionEvent e) {
				FileServer server = (FileServer) ((DeleteFolderActivity) node.getWorkflowObject()).getServer();
				String typeofServer = server.getTypeServ();

				if(typeofServer.equalsIgnoreCase(Messages.DeleteFolderSection_7)) {

					String url = server.getUrl();
					if(url.contains("{$VANILLA_HOME}")) { //$NON-NLS-1$
						MessageDialog.openInformation(Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell(), Messages.DeleteFolderSection_9, Messages.DeleteFolderSection_10);

					}
					if(url.contains("{$VANILLA_FILES}")) { //$NON-NLS-1$

						MessageDialog.openInformation(Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell(), Messages.DeleteFolderSection_12, Messages.DeleteFolderSection_13);
						...
						if(((FileServer) server).getRepertoireDef() != null) {
							cheminbuf.append(((FileServer) server).getRepertoireDef() + File.separator);
						}
						cheminbuf.append(select.getText() + File.separator);
						...
						nom.add(Messages.DeleteFolderSection_14);
					...
					try {
						...
						if(server.getRepertoireDef() != null) {
							ftp.changeWorkingDirectory(server.getRepertoireDef() + select.getText() + File.separator);
						}
						else {
							ftp.changeWorkingDirectory(select.getText() + File.separator);
						...
						ArrayList<String> nom = new ArrayList<String>();
						nom.add(Messages.DeleteFolderSection_17);
	...
	public void refresh() {
		...
		if(((DeleteFolderActivity) node.getWorkflowObject()).getPath() != null) {

			select.setText(((DeleteFolderActivity) node.getWorkflowObject()).getPath());
		}
		...
		public void widgetSelected(SelectionEvent e) {
			...
			if(server.getText() != null) {
				...
				if(s != null && s instanceof FileServer) {
					((DeleteFolderActivity) node.getWorkflowObject()).setServer(s);
