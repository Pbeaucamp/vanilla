...
import bpm.architect.web.server.servlets.UploadServlet;
...
public class UploadDialog extends AbstractDialogBox {
	...
	public UploadDialog(ConsultPanel parent, Contract contract) {
		...
		fileUploadPanel = new FileUploadWidget(CommonConstants.ARCHITECT_UPLOAD_SERVLET, parameters);
	...
	private void launchEtl() {
		CommonService.Connect.getInstance().getLinkedItems(contract.getId(), new GwtCallbackWrapper<List<DocumentItem>>(this, true) {
			...
			public void onSuccess(List<DocumentItem> result) {
				...
				if (result != null) {
					...
					if (nbEtl == 1) {
						...
						final String type = isEtl ? IRepositoryApi.BIG : IRepositoryApi.BIW;
						
						final InformationsDialog dial = new InformationsDialog(isEtl ? Labels.lblCnst.LaunchEtl() : Labels.lblCnst.LaunchWorkflow(), LabelsConstants.lblCnst.Confirmation(), LabelsConstants.lblCnst.Cancel(), isEtl ? Labels.lblCnst.WouldYouLikeToLaunchEtl() :  Labels.lblCnst.WouldYouLikeToLaunchWorkflow(), true);
						dial.center();
						dial.addCloseHandler(new CloseHandler<PopupPanel>() {
							...
							public void onClose(CloseEvent<PopupPanel> event) {
								if (dial.isConfirm()) {
									...
									ViewerDialog dial = new ViewerDialog(itemReport, parent.getInfoUser().getGroup());
									dial.center();
								}
							}
						});
