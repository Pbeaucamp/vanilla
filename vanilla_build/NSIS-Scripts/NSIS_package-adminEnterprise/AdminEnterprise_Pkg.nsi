...
;General
	...
	!define VSADM "VanillaServerAdministrator"
	!define VERSION "4.0"
	!define PCKG "AdminEnterprise"
	!define SERIE "Serie4"
	!define REG_ROOT "Software\Vanilla\${SERIE}\${PCKG}"
	
	;Name and file
	Name "Vanilla ${PCKG}"
	OutFile "VanillaAdminEnterprise_${VERSION}.exe"
...
Section "${VSADM}" SECTION_${VSADM}
		;If output folder allready exist ask for confirmation
		IfFileExists $INSTDIR\${SERIE}\${PCKG}\${VSADM} 0 doinstall
			MessageBox MB_YESNO "Directory allready exist at :$\r$\n $INSTDIR\${SERIE}\${PCKG}\${VSADM} $\r$\nDo you want to proceed ?" IDYES true IDNO false
			true:
				;Remove directory
				RMDir /r "$INSTDIR\${SERIE}\${PCKG}\${VSADM}"
				RMDir "$INSTDIR\${SERIE}\${PCKG}\${VSADM}"
				Goto doinstall
			...
			doinstall:
				;Set output path
				SetOutPath "$INSTDIR\${SERIE}\${PCKG}\${VSADM}"
				;Set sources path
				File /r "${VSADM}\*.*"
				;Write key registery for the installation dir and version
				WriteRegStr HKCU "${REG_ROOT}\Components\${VSADM}" "INST_LOC" "$INSTDIR\${SERIE}\${PCKG}\${VSADM}"
				WriteRegStr HKCU "${REG_ROOT}\Components\${VSADM}" "VERSION" "${VERSION}"
				
				;Create shortcut
				!insertmacro MUI_STARTMENU_WRITE_BEGIN Application
					CreateShortCut "$SMPROGRAMS\$StartMenuFolder\${PCKG}\${VSADM}.lnk" "$INSTDIR\${SERIE}\${PCKG}\${VSADM}\Bi ProcessManager.exe"
				!insertmacro MUI_STARTMENU_WRITE_END
				
				;Write key registery for shortcutPath
				WriteRegStr HKCU "${REG_ROOT}\Components\${VSADM}" "SHORT_LOC" "$SMPROGRAMS\$StartMenuFolder\${PCKG}\${VSADM}.lnk"
	...
	LangString DESC_${VSADM} ${LANG_ENGLISH} "Install Vanilla ${VSADM} ${VERSION}"
	...
	!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
	  ...
	  !insertmacro MUI_DESCRIPTION_TEXT ${SECTION_${VSADM}} $(DESC_${VSADM})
...
Section "un.${VSADM}"  un.${VSADM}
	ReadRegStr $0 HKCU "${REG_ROOT}\Components\${VSADM}" "INST_LOC"
	RMDir /r "$0"
	RMDir "$0"	
	DeleteRegKey HKCU "${REG_ROOT}\Components\${VSADM}\INST_LOC"
	DeleteRegKey HKCU "${REG_ROOT}\Components\${VSADM}\VERSION"	
	ReadRegStr $1 HKCU "${REG_ROOT}\Components\${VSADM}" "SHORT_LOC"
	Delete "$1"
	DeleteRegKey HKCU "${REG_ROOT}\Components\${VSADM}\VERSION\SHORT_LOC"	
	DeleteRegKey HKCU "${REG_ROOT}\Components\${VSADM}"
SectionEnd	
	...
	LangString UDESC_${VSADM} ${LANG_ENGLISH} "Uninstall Vanilla ${VSADM} ${VERSION}"
	...
	!insertmacro MUI_UNFUNCTION_DESCRIPTION_BEGIN
	  ...
	  !insertmacro MUI_DESCRIPTION_TEXT ${un.${VSADM}} $(UDESC_${VSADM})
