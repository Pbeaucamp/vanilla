...
!define APP_NAME "vanilla-tomcat-hsql-2.0-GA-ce"
!define APP_Version ""
Name ${APP_NAME}
...
!define PRODUCT_INSTALL_DIR "$PROGRAMFILES\${APP_NAME}"
...
!insertmacro MUI_PAGE_LICENSE "vanilla-tomcat-hsql-2.0-GA-ce\License.txt"
...
#Configuratioon de la finish page avec creation ou pas d'un "desktop shortcut"
	...
	!define MUI_FINISHPAGE_LINK "Visit the Vanilla Tomcat For Hsql 2.0-GA-ce website for the latest updates."
...
#Copie des fichier du RCP
  SetOutPath "$INSTDIR"
  File /r "vanilla-tomcat-hsql-2.0-GA-ce\*.*"
...
#creation des raccourcis
	...
	CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\start-vanilla.lnk" "$INSTDIR\start-vanilla.bat"
	CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\stop-vanilla.lnk" "$INSTDIR\stop-vanilla.bat"
...
Section Documentation
		...
		;File "BIProfiler\Readme.txt"
		File "vanilla-tomcat-hsql-2.0-GA-ce\License.txt"
...
Section Uninstall
  ...
  Delete "$DESKTOP\start-vanilla.lnk" 
  Delete "$DESKTOP\stop-vanilla.lnk"
  ...
  Delete "$SMPROGRAMS\$STARTMENU_FOLDER\start-vanilla.lnk"
  Delete "$SMPROGRAMS\$STARTMENU_FOLDER\stop-vanilla.lnk"
...
Function CreateDeskLink
	...
	CreateShortCut "$DESKTOP\start-vanilla.lnk" "$INSTDIR\start-vanilla.bat" ""
	CreateShortCut "$DESKTOP\stop-vanilla.lnk" "$INSTDIR\stop-vanilla.bat" ""

	nsExec::Exec "$DESKTOP\start-vanilla.lnk"
	nsExec::Exec "$DESKTOP\stop-vanilla.lnk" 
