...
# Jetty's hallmark
##################################################
JETTY_INSTALL_TRACE_FILE="vanilla/VanillaRuntime/org.eclipse.osgi_3.13.200.v20181130-2106.jar"
...
then
  JETTY_START_LOG="$JETTY_RUN/runtime/$NAME-start.log"
fi
...
if [ -z "$JETTY_PID" ]
then
  JETTY_PID="$JETTY_RUN/runtime/${NAME}.pid"
fi

if [ -z "$JETTY_STATE" ]
then
  JETTY_STATE="$JETTY_RUN/runtime/${NAME}.state"
fi
...
JETTY_START=$JETTY_HOME/vanilla/VanillaRuntime/org.eclipse.osgi_3.13.200.v20181130-2106.jar
START_INI=$JETTY_BASE/bin-runtime/start.ini
...
case "$ACTION" in
  start)
    echo -n "Starting Vanilla (Runtime): "
    ...
    then
      ...
      PROG_ARGS="-Xms1024m -Xmx2048m -Dosgi.compatibility.bootdelegation=true -Declipse.ignoreApp=true -Dosgi.noShutdown=true -Dlog4j.debug=true -Dorg.eclipse.equinox.http.jetty.http.port=7171 -Dorg.eclipse.equinox.http.jetty.context.path=/VanillaRuntime -Dbpm.vanilla.configurationFile=$JETTY_BASE/vanilla/vanilla-conf/vanilla.properties -Dlogback.configurationFile=$JETTY_BASE/vanilla/vanilla-conf/logback.xml -jar $JETTY_BASE/vanilla/VanillaRuntime/org.eclipse.osgi_3.13.200.v20181130-2106.jar jetty-started.xml"
      # start-stop-daemon -S -p"$JETTY_PID" $CH_USER -d"$JETTY_BASE" -b -m -a $JAVA -- $PROG_ARGS
      start-stop-daemon --start --pidfile "$JETTY_PID" --make-pidfile --chuid $UID -d"$JETTY_BASE" --background --no-close --startas /bin/bash -- -c "exec java $PROG_ARGS > ./logs/runtime.out 2>&1"
      ...
      then
        ...
        su - "$JETTY_USER" $SU_SHELL -c "
          cd \"$JETTY_BASE\"
          exec ${RUN_CMD[*]} start-log-file=\"$JETTY_START_LOG\" > ./logs/runtime.out 2>&1
          disown \$!
          echo \$! > \"$JETTY_PID\""
      else
        "${RUN_CMD[@]}" > /dev/null &
  ...
  stop)
    echo -n "Stopping Vanilla (Runtime): "
  ...
  restart)
    ...
    > "$JETTY_STATE"
    if [ ! -f $JETTY_SH ]; then
      if [ ! -f $JETTY_HOME/bin-runtime/jetty.sh ]; then
        echo "$JETTY_HOME/bin-runtime/jetty.sh does not exist."
        exit 1
      fi
      JETTY_SH=$JETTY_HOME/bin-runtime/jetty.sh
    fi
