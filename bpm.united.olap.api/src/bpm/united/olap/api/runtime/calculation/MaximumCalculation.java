...
public class MaximumCalculation implements ICalculation {
	...
	public MaximumCalculation(DataCell cell, Measure measure) {
		this.cell = cell;
...
//			Double result = Collections.max(cell.getValuesByKey(measure.getItem().getName()));
...
//		}
	...
	public void makeCalculDuringQuery(boolean isOnImprovedQuery) {
		if(cell.getValues() != null && cell.getValuesByKey(measure.getUname()) != null && cell.getValuesByKey(measure.getUname()).size() > 0) {
			Double result = Collections.max(cell.getValuesByKey(measure.getUname()));
