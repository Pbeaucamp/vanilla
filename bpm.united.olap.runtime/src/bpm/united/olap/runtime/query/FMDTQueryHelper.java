...
import bpm.united.olap.runtime.query.improver.FMDTQueryImprover2;
...
public class FMDTQueryHelper extends QueryHelper {
	private List<DataCellIdentifier2> failurePossibleIds = new ArrayList<DataCellIdentifier2>();
	public FMDTQueryHelper(IVanillaLogger logger, IRuntimeContext runtimeContext) {
	...
	public DataStorage executeQuery(ICubeInstance cubeInstance, DataStorage storage, int limit) throws Exception {
		...
		for(DataCellIdentifier2 possibleId : storage.getPossibleIds().keySet()) {
			FMDTQueryImprover2 queryImprover = new FMDTQueryImprover2(logger, runtimeContext);
			...
			try{
				...
				odaInput.setDatasetPrivateProperties((Properties)input.getDatasetPrivateProperties().clone());
				odaInput.setDatasetPublicProperties((Properties)input.getDatasetPublicProperties().clone());
				odaInput.setDatasourcePrivateProperties((Properties)input.getDatasourcePrivateProperties().clone());
				odaInput.setDatasourcePublicProperties((Properties)input.getDatasourcePublicProperties().clone());
				odaInput.setOdaExtensionDataSourceId(input.getOdaExtensionDataSourceId());
				odaInput.setOdaExtensionId(input.getOdaExtensionId());
				
				queryOda = getOdaQuery(odaInput, improvedQuery);
