...
public class CompositeVisbilityHelper {
	private class Secu{
	...
	private IDataStreamElement datas;
	private FormToolkit toolkit;
	
	public CompositeVisbilityHelper(FormToolkit toolkit, Composite parent, int style) {
	...
	private Control createToolbar(final Composite parent){
		...
		checkAll.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				for(Object o : ((IStructuredContentProvider)tableGroups.getContentProvider()).getElements(tableGroups.getInput())){
					datas.setVisible(((Secu)o).groupName, true);
				}
		...
		uncheckAll.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				for(Object o : ((IStructuredContentProvider)tableGroups.getContentProvider()).getElements(tableGroups.getInput())){
					datas.setVisible(((Secu)o).groupName, false);
				}
				parent.notifyListeners(SWT.Selection, new Event());
			}
		});
	...
	private void createContent(final Composite main){
		final Composite parent = toolkit.createComposite(main, SWT.NONE);
		...
		tableGroups = new CheckboxTreeViewer(toolkit.createTree(parent, SWT.V_SCROLL  | SWT.VIRTUAL | SWT.CHECK));
		...
		tableGroups.addCheckStateListener(new ICheckStateListener() {
			...
			public void checkStateChanged(CheckStateChangedEvent event) {
				...
				datas.setVisible(s.groupName, b);
				parent.notifyListeners(SWT.Selection, new Event());
			}
		});
	...
	public void setDatas(IDataStreamElement datas){
		this.datas = datas;
		fill();
	}
	...
	private void fill(){
		for(Secu s : (List<Secu>)tableGroups.getInput()){
			if (datas.isVisibleFor(s.groupName)){
				s.visible = true;
