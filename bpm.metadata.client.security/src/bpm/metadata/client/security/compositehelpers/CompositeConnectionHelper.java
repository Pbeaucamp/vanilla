...
public class CompositeConnectionHelper {
	...
	public CompositeConnectionHelper(FormToolkit toolkit, Composite parent, int style) {
	...
	protected Control createToolbar(final Composite parent){
		...
		checkAll.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				for(Object o : ((IStructuredContentProvider)tableGroups.getContentProvider()).getElements(tableGroups.getInput())){
					dataSource.securizeConnection(connection.getName(), ((Secu)o).groupName, true);
				}
		...
		uncheckAll.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				for(Object o : ((IStructuredContentProvider)tableGroups.getContentProvider()).getElements(tableGroups.getInput())){
					dataSource.securizeConnection(connection.getName(), ((Secu)o).groupName, false);
				}
	...
	protected void createGroupViewer(final Composite main){
		...
		tableGroups.addCheckStateListener(new ICheckStateListener() {
			...
			public void checkStateChanged(CheckStateChangedEvent event) {
				...
				dataSource.securizeConnection(connection.getName(), s.groupName, b);
	...
	public void setConnection(IConnection datas){
		this.connection = datas;
		for(IDataSource ds : Activator.getDefault().getModel().getDataSources()){
			if (ds.getConnections(null).contains(datas)){
				this.dataSource = (AbstractDataSource)ds;
				break;
			}
		}
	...
	protected void fill(){
		...
		for(Secu s : (List<Secu>)tableGroups.getInput()){
			List<IConnection> l = dataSource.getConnections(s.groupName);
