...
public class UserPanel extends PopupPanel implements IChangeTheme {
	...
	public UserPanel(InfoUser infoUser) {
		...
		imgAbout.setUrl(WysiwygImage.INSTANCE.about_me().getSafeUri());
	...
	public void fillDataUser(User portalUser, Repository rep, Group group){
		...
		lblEmail.setText(portalUser.getBusinessMail() == null || portalUser.getBusinessMail().isEmpty() ? Bpm_fwr.LBLW.UnknownMail() : portalUser.getBusinessMail());
		
		lblName.setText(portalUser.getName());
		lblUserEmail.setText(portalUser.getBusinessMail() == null || portalUser.getBusinessMail().isEmpty() ? Bpm_fwr.LBLW.UnknownMail() : portalUser.getBusinessMail());
