...
public class TsbnAppDaoComponent {

	public static final String BEAN_SCHEDULER_DWH_APP = "dwhAppDAO";

	private DwhAppDAO dwhAppDao;

	public void init(String driver, String dbUrl, String username, String password) throws Exception {
		ClassPathResource configFile = new ClassPathResource("/bpm/gateway/core/tsbn/db/bpm.tsbn.app.context.xml", TsbnAppDaoComponent.class.getClassLoader());
		...
		try {
			dwhAppDao = (DwhAppDAO) factory.getBean(BEAN_SCHEDULER_DWH_APP);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	public DwhAppDAO getDwhAppDao() {
		return dwhAppDao;
	}
}
