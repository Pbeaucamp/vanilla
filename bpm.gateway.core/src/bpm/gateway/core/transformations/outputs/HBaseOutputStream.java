...
import bpm.gateway.runtime2.transformations.outputs.RunHBaseOutput;
import bpm.vanilla.platform.core.IRepositoryContext;

public class HBaseOutputStream extends AbstractTransformation implements INoSQLStream, IOutputNoSQL, Trashable{
	...
	public Element getElement() {
		Element e = DocumentHelper.createElement("hbaseOutputStream");
	...
	public void deleteMapping(Transformation input, String inputCol){
		...
		if(maps != null && maps.get(inputCol) != null){
	...
	public void refreshDescriptor() {
		...
		if (lastTableName != tableName || server != lastLoadedServer 
				|| lastDefinition != definition || lastColumnFamily != columnFamily){
			try {
				if(getTableName() != null && getColumnFamily() != null){
					descriptor = HBaseHelper.buildDescriptor(getTableName(), getColumnFamily(), this);
				}
				else {
	...
	public RuntimeStep getExecutioner(IRepositoryContext repositoryCtx, int bufferSize) {
		return new RunHBaseOutput(this, bufferSize);
	}
	...
	public Transformation copy() {
		HBaseOutputStream copy = new HBaseOutputStream();
