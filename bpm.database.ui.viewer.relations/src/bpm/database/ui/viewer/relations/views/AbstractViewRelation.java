...
abstract public class AbstractViewRelation extends ViewPart implements ISelectionListener{
	...
	private void createThumbnail(Composite parent){
		...
		ScrollableThumbnail thumbnail = new ScrollableThumbnail((Viewport) ((ScalableRootEditPart) editor.getRootEditPart()).getFigure());
		thumbnail.setSource(((ScalableRootEditPart) editor.getRootEditPart()).getLayer(LayerConstants.PRINTABLE_LAYERS));
...
//				zoomManager.setViewLocation(zoomManager.getViewport().getLocation());
				if (zm.canZoomIn()) {
					zm.zoomIn();
				}
			}
...
//				zoomManager.setViewLocation(zoomManager.getViewport().getLocation());
		...
		saveAsPicture.addSelectionListener(new SelectionAdapter(){
			...
			public void widgetSelected(SelectionEvent e) {
				...
				IFigure figure = ((ScalableRootEditPart)editor.getRootEditPart()).getLayer(LayerConstants.SCALABLE_LAYERS);
	...
	protected void initEditor(Composite parent){
		...
		FmdtRootPart rootEditPart = new FmdtRootPart();
