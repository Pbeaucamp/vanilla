...
public class GroupViewerView extends ViewPart {
	...
	public static final String DATA_PROP = "Data change"; //$NON-NLS-1$
	public static final String ID = "groupviewer.views.GroupViewerView"; //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
		...
		itemFlex.setText("Flex"); //$NON-NLS-1$
	...
	private void createActions() {
		/*
		 * this.export = new Action("Export to FLEX",
		 * getImageDescriptor("flex-ico.png")){
		 * 
		 * @Override public void run() { doExport(); } };
		 */

		this.refresh = new Action(Messages.GroupViewerView_0, getImageDescriptor("refresh.png")) {//$NON-NLS-1$
			@Override
			public void run() {
				doRefresh();
			}
	...
	/*
	 * private void doExport() { // Instantiates and initializes the wizard
	 * ExportWizard wiz = new ExportWizard();
	 * wiz.setModel(ExportModel.getInstance()); WizardDialog dialog = new
	 * WizardDialog(getSite().getShell(), wiz); dialog.create(); dialog.open();
	 * 
	 * if (wiz.performFinish()){ generateXML(); browse.refresh(); }
	 * 
	 * }
	 */
