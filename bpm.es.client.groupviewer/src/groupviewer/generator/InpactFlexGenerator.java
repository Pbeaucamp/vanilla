...
public class InpactFlexGenerator extends FlexGenerator {
	...
	private void generateSubFlex(ImpactLevel parent, Node parentNode) {
		for(ImpactLevel level : parent.getChildren()) {
			for(Object elem : level.getElements()) {
				
				RepositoryItem item = null;
				if(elem instanceof RepositoryItem) {
					item = (RepositoryItem) elem;
				}
				else {
					item = createFakeItemForDatasource((AbstractDatasource) elem);
				}
				Node sub = createNode(item);
				addNode(sub);
				sub.addParentNode(parentNode);
				generateSubFlex(level, sub);
			}
		}
	}
	
	public void generateFlexThing(List<ImpactLevel> levels) {
		for(ImpactLevel level : levels) {
			for(Object elem : level.getElements()) {
				RepositoryItem item = null;
				if(elem instanceof RepositoryItem) {
					item = (RepositoryItem) elem;
	...
	private RepositoryItem createFakeItemForDatasource(AbstractDatasource datasource) {
		RepositoryItem fake = new RepositoryItem();
		fake.setType(8);
		fake.setSubtype(1);
		fake.setItemName(datasource.getName());
		return fake;
	}
	...
	private Node createNode (RepositoryItem item){
		...
		Node node = new Node(item.getItemName(),item.getComment() ,type ,"16"); //$NON-NLS-1$
		node.setData(item);
		return node;
	}
	...
	public Map<Class<?>, List<FlexProperty>> createExtraAtrributes() {
		...
		usersProp.add(new FlexProperty("itemId", "getId"));// //$NON-NLS-1$ //$NON-NLS-2$
		//usersProp.add(new FlexProperty("isDeportable", "getIsDeportable"));
		//usersProp.add(new FlexProperty("isOn", "getIsOn"));
		usersProp.add(new FlexProperty("modification", "getModification"));// //$NON-NLS-1$ //$NON-NLS-2$
		//usersProp.add(new FlexProperty("modifiedBy", "getModifiedBy"));
		usersProp.add(new FlexProperty("modifier", "getModifier"));// //$NON-NLS-1$ //$NON-NLS-2$
		usersProp.add(new FlexProperty("type", "getType"));// //$NON-NLS-1$ //$NON-NLS-2$
		usersProp.add(new FlexProperty("creation", "getCreation"));// //$NON-NLS-1$ //$NON-NLS-2$
		usersProp.add(new FlexProperty("creator", "getCreator"));// //$NON-NLS-1$ //$NON-NLS-2$
