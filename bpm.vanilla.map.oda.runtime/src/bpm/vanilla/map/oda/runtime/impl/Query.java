...
public class Query implements IQuery
{
    ...
    private long fusionMapObjectId = -1;
    
    
    private IFusionMapRegistry fusionMapRegistry;
    public Query(IFusionMapRegistry fusionMapRegistry){
    	this.fusionMapRegistry = fusionMapRegistry;
    }
	/*
	...
	public void prepare( String queryText ) throws OdaException
	{
		...
		try{
			fusionMapObjectId = Long.parseLong(queryText);
			m_preparedText = queryText;
			
		}catch(Exception ex){
			throw new OdaException(new Exception("Unable to parse queryText {" + queryText + "} - " + ex.getMessage(), ex));
		}
	...
	public IResultSet executeQuery() throws OdaException
	{
		...
		IFusionMapObject object = null;
		
		try{
			object = fusionMapRegistry.getFusionMapObject(fusionMapObjectId);
		}catch(Exception ex){
			throw new OdaException(new Exception("Error when getting FusionMap Object from vanillaServer - " + ex.getMessage(), ex));
		}
		
		
		IResultSet resultSet = new ResultSet(object.getSpecificationsEntities());
